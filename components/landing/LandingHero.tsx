...
import { Sparkles, ScanLine, TrendingUp, ArrowUpRight, Check } from 'lucide-react'
import { useLanguage } from '@/contexts/LanguageContext'
import { getLandingCopy } from './copy'
import { DemoButton, contactHref } from './LandingCta'

// Faux dashboard bars — decorative, height %s only.
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div
        ...
        style={{ backgroundImage: 'radial-gradient(rgb(99 102 241 / 0.07) 1px, transparent 1px)', backgroundSize: '22px 22px' }}
      />
      <div aria-hidden className="pointer-events-none absolute -top-32 left-1/2 h-80 w-[44rem] -translate-x-1/2 rounded-full bg-indigo-300/30 blur-3xl" />

      <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-4 py-14 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:py-20">
        {/* ── Copy ── */}
        <div>
          <a href="#pricing" className="animate-fade-up inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 transition-colors hover:bg-amber-100">
            <Sparkles size={13} /> {c.hero.offer}
          </a>

          <p className="animate-fade-up mt-4 text-xs font-bold uppercase tracking-widest text-indigo-500" style={{ animationDelay: '40ms' }}>
            {c.hero.badge}
          </p>

          <h1 className="animate-fade-up mt-2 text-[2.1rem] font-bold leading-[1.07] tracking-tight text-gray-900 sm:text-5xl lg:text-[3.4rem]" style={{ animationDelay: '80ms' }}>
            {c.hero.h1a}{' '}
            <span className="bg-gradient-to-r from-indigo-600 to-violet-500 bg-clip-text text-transparent">{c.hero.accent}</span>{' '}
            {c.hero.h1b}
          </h1>
          ...
          <p className="animate-fade-up mt-2 max-w-xl text-sm text-gray-400" style={{ animationDelay: '150ms' }}>
            {c.hero.zhNote}
          </p>

          <div className="animate-fade-up mt-7 flex flex-wrap items-center gap-3" style={{ animationDelay: '190ms' }}>
          ...
          <p className="animate-fade-up mt-3 text-xs text-gray-400" style={{ animationDelay: '220ms' }}>
            {c.hero.micro}
          </p>

          <ul className="animate-fade-up mt-6 flex flex-wrap gap-x-5 gap-y-2" style={{ animationDelay: '260ms' }}>
            {c.hero.chips.map((chip) => (
              <li key={chip} className="flex items-center gap-1.5 text-[13px] font-medium text-gray-600">
        ...
        <div className="animate-fade-up relative mx-auto w-full max-w-md lg:mx-0" style={{ animationDelay: '180ms' }}>
          <div className="relative rotate-1 overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 p-6 shadow-hero ring-1 ring-white/10">
            <div aria-hidden className="pointer-events-none absolute inset-0 opacity-50" style={{ backgroundImage: 'radial-gradient(rgb(255 255 255 / 0.05) 1px, transparent 1px)', backgroundSize: '16px 16px' }} />
            <div className="relative">
              ...
              <div className="mt-6 flex h-28 items-end gap-2">
                {BARS.map((h, i) => (
                  <div key={i} className={`flex-1 rounded-t-md ${i === BARS.length - 1 ? 'bg-indigo-400' : 'bg-white/15'}`} style={{ height: `${h}%` }} />
                ))}
              </div>
            </div>
          </div>
